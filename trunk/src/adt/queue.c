...
void queue_free($$ PQueue *Q, PDelegate free_elm ) { $H
    ...
    gen_list_free($$A Q->_head, free_elm );
    gen_list_free($$A Q->_unused, &delegate_do_nothing );
    mem_free(Q);
...
void queue_push($$ PQueue * const Q, void * E ) { $H
    ...
    /* take the first unused one otherwise */
        ...
        L = Q->_unused;
        Q->_unused = (PGenericList *) list_get_next($$A L);
    }

    /* add in the slot to the tail of the queue */
    if(NULL != Q->_tail) {
        list_set_next($$A Q->_tail, L );
...
void *queue_pop($$ PQueue * const Q ) { $H
    ...
    /* update the head pointer */
        ...
        Q->_head = (PGenericList *) list_get_next($$A L);
    }
