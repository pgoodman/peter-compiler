...
void tree_free($$ void *T, PDelegate free_tree_fnc ) { $H
    ...
    free_tree_fnc = T_valid_free_callback($$A
        free_tree_fnc,
     ...
     * bottom up. */
    G = tree_generator_alloc($$A T, TREE_TRAVERSE_POSTORDER );

    while(generator_next($$A G)) {
        elm = generator_current($$A G );
        node = (PTree *)elm;
...
void PDelegateree_free($$ void *T ) { $H
    tree_free($$A T, &delegate_do_nothing );
    return_with;
}
...
void tree_trim($$ void *tree, PStack *garbage_stack ) { $H
    ...
    for(i = 0; i < T->_fill; ++i) {
        ...
        stack_push($$A garbage_stack, branch );
...
static void *T_generator_next_df($$ void *g ) { $H
    ...
    while(!stack_is_empty($$A S)) {
        ...
        for(i = curr->_fill; i > 0; ) {
            stack_push($$A S, curr->_branches[--i] );
        }
...
static void *T_generator_next_bf($$ void *g ) { $H
    ...
    while(!queue_is_empty($$A Q)) {
        ...
        for(i = 0; i < curr->_fill; ++i) {
            queue_push($$A Q, curr->_branches[i] );
        }
...
PTreeGenerator *tree_generator_alloc($$ void *tree,
    ...
    PTree *T = (PTree *) tree;
    PTreeGenerator *G = generator_alloc($$A sizeof(PTreeGenerator));
    ...
    switch(traverse_type) {
        ...
        case TREE_TRAVERSE_POSTORDER:
            G->_adt = stack_alloc($$A sizeof(PStack));
            generator_init($$A G, \
                &T_generator_next_po, \
                &T_generator_free_s \
            );
            stack_push($$A G->_adt, T );
            break;
        ...
        case TREE_TRAVERSE_LEVELORDER:
            G->_adt = queue_alloc($$A sizeof(PQueue) );
            generator_init($$A G, \
                &T_generator_next_bf, \
                &T_generator_free_q \
            );
            queue_push($$A G->_adt, T );
            break;
    }
