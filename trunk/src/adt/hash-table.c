 ...
 */
static void H_grow(HashTable *H $$) { $H
    assert(NULL != H);
    ...
    slots = H_alloc_slots(new_size _$$);
 ...
 */
void *gen_hash_table_alloc(size_t size, const uint32_t num_slots $$) { $H
    ...
    elms = H_alloc_slots(num_slots _$$);
 ...
 */
HashTable *hash_table_alloc(const uint32_t num_slots $$) { $H
    return_with (HashTable *) gen_hash_table_alloc(0, num_slots _$$);
}
...
void hash_table_set(HashTable *H, void *key, void *val,
                    D1_t free_on_overwrite_fnc $$) { $H

    assert(NULL != H \
        && NULL != key \
        && NULL != val \
        && free_on_overwrite_fnc != NULL);
    ...
    if(NULL != H->elms[h])
        free_on_overwrite_fnc(H->elms[h] _$$);
 ...
 */
void hash_table_unset(HashTable *H, void *c, D1_t free_fnc $$) { $H
